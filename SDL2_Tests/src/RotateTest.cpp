...
struct RotateSprite {
    ...
    float angle;       // K¹t o jaki obraca siê sprite
};

// Implementacja metody run uruchamiaj¹cej test
void RotateTest::run(SDL_Renderer* renderer) {
    ...
    for (int count : thresholds) {
        ...
        // Inicjalizujemy sprite'y z losow¹ pozycj¹ i k¹tem obrotu równym 0
        for (int i = 0; i < count; ++i) {

            // Ustawiamy losow¹ pozycjê sprite'a
            int x = xDist(rng);
            int y = yDist(rng);
            sprites.push_back({ SDL_Rect{ x, y, texWidth, texHeight }, 0.0f });
        }
        ...
        while (true) {
            ...
            // Obracamy i renderujemy ka¿dy sprite
            for (auto& s : sprites) {
                SDL_RenderCopyEx(renderer, texture, nullptr, &s.destRect, s.angle, nullptr, SDL_FLIP_NONE);
                s.angle += 1.0f; // Zwiêkszamy k¹t obrotu o 1 stopieñ
            }
