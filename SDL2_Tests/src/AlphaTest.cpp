...
struct AlphaSprite {
    SDL_Rect destRect; // Prostok¹t docelowy, okreœlaj¹cy pozycjê i rozmiar sprite'a
};

// Implementacja metody run uruchamiaj¹cej test
void AlphaTest::run(SDL_Renderer* renderer) {
    ...
    // Ustawiamy 50% przezroczystoœci dla tekstury
    ...
    int texWidth, texHeight; // Zmienne pod d³ugoœæ i wysokoœæ
    ...
    for (int count : thresholds) {
        ...
        while (true) {
            ...
            // Renderujemy wszystkie sprite'y wraz z ich przezroczystoœci¹
